...
export class AdminRepository {
  ...
  async getPlatformMetrics() {
    const gymsRes = await this.db
      .prepare(`SELECT COUNT(*) as total_gyms, SUM(CASE WHEN status = 'ACTIVE' THEN 1 ELSE 0 END) as active_gyms FROM gyms WHERE deleted_at IS NULL`)
      .first<any>();
    ...
    const revRes = await this.db
      .prepare(`SELECT SUM(amount) as platform_revenue FROM payments WHERE status = 'COMPLETED'`)
      .first<any>();
