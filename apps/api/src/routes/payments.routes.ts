...
export function registerPaymentRoutes(router: NativeRouter): void {
  // List payments
  router.get('/api/payments', async (req, ctx) => {
    const tenant = await requireGym(req, ctx);
    if (tenant instanceof Response) return tenant;

    const featErr = await requireFeature(req, ctx, 'payments');
  ...
  router.post('/api/payments', async (req, ctx) => {
    ...
    if (!parsed.success) {
      return errorResponse(parsed.error.errors[0]?.message || 'Invalid payment payload', 400);
    }
    ...
    if (!member || member.deleted_at !== null || member.status === 'BLOCKED') {
      return errorResponse('Cannot record payment for an inactive, archived, or blocked member', 400);
    }
    ...
    const whatsappUrl = notif.generateWhatsAppUrl({
      ...
      params: {
        amount: parsed.data.amountPaise / 100,
        paymentMode: parsed.data.paymentMode,
        receiptNumber,
      },
    });

    await auditGym(ctx, 'payment.create', 'payment', paymentId, {
  ...
  router.get('/api/payments/:id/invoice', async (req, ctx) => {
    ...
    return json({
      ...
      gym: {
        name: tenant.gym.name,
        address: tenant.gym.address,
        city: tenant.gym.city,
        state: tenant.gym.state,
        pincode: tenant.gym.pincode,
        phone: tenant.gym.phone,
        email: tenant.gym.email,
        gstNumber: tenant.gym.gst_number,
      },
      member: {
