...
import { requireGym, requireFeature, requireRole, TenantResolution } from '../lib/tenant';
import { DashboardService } from '../services/dashboard.service';

export function registerReportRoutes(router: NativeRouter): void {
  // Get financial report
  router.get('/api/reports', async (req, ctx) => {
    const tenant = await requireGym(req, ctx);
    if (tenant instanceof Response) return tenant;

    const featErr = await requireFeature(req, ctx, 'reports');
    ...
    const dashboardService = new DashboardService(ctx.env.DB, ctx.gymId!, (tenant as TenantResolution).gym.name);
