// Lightweight Web Crypto based session & password utilities (Edge & Cloudflare Workers compatible)
...
export async function createSessionToken(user: Omit<UserSessionPayload, 'exp'>, secret: string, expiresInSeconds = 86400 * 7): Promise<string> {
...
export async function verifySessionToken(token: string, secret: string): Promise<UserSessionPayload | null> {
  try {
