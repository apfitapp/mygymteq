...
interface CheckInPanelProps {
  onCheckIn: (code: string, method?: 'MANUAL' | 'QR_SCAN') => Promise<void>;
  isCheckingIn: boolean;
  errorMessage: string | null;
  blockedMember: { id?: string; name?: string; expiryDate?: string } | null;
  lastCheckedMember: {
    name: string;
    code: string;
...
export const CheckInPanel: React.FC<CheckInPanelProps> = ({
  ...
  const [activeMode, setActiveMode] = useState<'dialpad' | 'camera'>('dialpad');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (inputCode.trim()) {
      onCheckIn(inputCode.trim(), 'MANUAL');
      setInputCode('');
    }
  };
        ...
        {activeMode === 'camera' && (
          <div className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-border rounded-xl bg-secondary/40 gap-3 text-center">
            <div className="relative size-44 rounded-xl border-2 border-primary bg-black/10 flex items-center justify-center overflow-hidden">
              <div className="absolute inset-x-0 top-0 h-0.5 bg-primary animate-[bounce_2s_infinite]" />
